...
forecast = forecast.Arima(auto_arima_data, h=1000)
...
plot(decompose(ts(coredata(data.z), frequency=24),xlab="Day")
title(sub=paste(sub_text,collapse="  to  "))
```


``` {r fig.height=12,fig.width=12, error=TRUE }
plot(decompose(ts(coredata(data.z), frequency=168)),xlab="Week")
title(sub=paste(sub_text,collapse="  to  "))
```
